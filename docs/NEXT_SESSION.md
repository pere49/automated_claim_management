...
- **Git:** branch `backend`; pushed through `5c460e0`; **`e202fa6` (Stage C before the rework) committed locally, not pushed**; the rework is uncommitted. Commit and push only when the user asks; never force; privacy sweep first (a sweep before `e202fa6` found one real file-name prefix in this note, replaced with "…").
