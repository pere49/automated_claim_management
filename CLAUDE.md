...
- app/ — the application. `python -m app` or start_app.bat starts it.
  ...
  - app/claims/ — reading a claim sheet: a PDF of it (text layer and ruled lines) or a scanned picture (OCR words placed in the lines found in the picture) — Excel is not read for now (D39); every amount from the Motor to the Other column (D40); dates by rule R10u; where each row and cell sits on the page (SheetPlace, for the sheet view); rules in claim_rules.json. The readers load lazily, so the decision package can use its records without image code.
  - app/checking/ — THE Stage C decision package: PIN scan, repeated pages, amount index, total anchor (A4m+), pairing (a paired page leaves every later search), links (one page per amount, by date only when certain), badges and colour rule B (red = no valid receipt), PASS / CAUTION / REVIEW with reasons, the Total status; rules in checking_rules.json. Imports nothing from OCR, image or GUI code (a test enforces it). Check it on the real claims with tools/stage_c_acceptance/run_acceptance.py.
  - app/company_pin.py — the company's Kenyan PIN from the git-ignored .env (never shown, logged or committed).
  - app/gui/ — the PySide6 review window, one file per pane plus the background OCR worker, the open-document session, the page preparer (every read page tokenised once for the checking), the claim session, the tour and highlight outlines. Receipts side: receipts_header.py ("Receipt claim:", the PIN toggle — toggle_switch.py —, Auto, Next to check), page badges (page_badges.py). Claim side: the sheet shown as its PDF (sheet_view.py, with sheet_canvas.py, sheet_overlay.py, row_buttons.py) and the status cards under it (status_cards.py, elided_label.py). What a checked claim shows on the receipts: claim_presenter.py. No OCR text or Search panel (removed, D42). Settings in gui_settings.json.
- tools/ — prototypes and measurement trials (each says so in its docstring): ocr_resolution_trial, search_criteria_trial (+ cents_rule.py), date_parsing_trial, duplicate_pages_trial, total_anchor_trial, stage_c_timing, stage_c_dryrun (the checking prototype), badge_colour_trial (colour rule A vs B on the real claims), stage_c_acceptance (the real claims through the application). Their answer keys live in private/ folders and their results in outputs/, both ignored by git.
